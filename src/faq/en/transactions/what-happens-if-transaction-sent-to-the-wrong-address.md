...
Safecoinapp wallet can validate destination addresses and warn a sender if the entered destination address doesn't match the typical format for a payment address on that blockchain. For instance, the wallet won't let you send Bitcoin to an Ethereum address and so on.

Also, Safecoinapp can alert sender when s/he mistypes or enters an incomplete payment address.